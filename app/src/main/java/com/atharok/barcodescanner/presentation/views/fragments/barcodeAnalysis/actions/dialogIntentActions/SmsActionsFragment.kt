...
class SmsActionsFragment: AbstractDialogIntentActionsFragment() {

    override fun start(barcode: Barcode, parsedResult: ParsedResult) {

        addSearchWithEngineActionFAB(barcode.contents)

        if(parsedResult.type == ParsedResultType.SMS) {
            val actionsArray = arrayOf(ActionEnum.ADD_SMS_NUMBER, ActionEnum.CALL_SMS_NUMBER, ActionEnum.SEND_SMS)
