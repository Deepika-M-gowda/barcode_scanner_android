...
class ProductActionsFragment: AbstractSearchDialogIntentActionsFragment() {

    override fun start(barcode: Barcode, parsedResult: ParsedResult) {
        if(parsedResult.type == ParsedResultType.PRODUCT && barcode.getBarcodeType() == BarcodeType.UNKNOWN_PRODUCT) {
            configureProductSearchFAB(barcode.contents)
        }
    }

    private fun configureProductSearchFAB(contents: String){
