...
class SmsActionsFragment: AbstractActionsFragment() {
    override fun configureActions(barcode: Barcode, parsedResult: ParsedResult): Array<ActionItem> {
        return when(parsedResult){
            is SMSParsedResult -> configureSmsActions(barcode.contents, parsedResult)
            else -> configureDefaultActions(barcode.contents)
        }
    }

    private fun configureSmsActions(contents: String, parsedResult: SMSParsedResult) = arrayOf(
    ...
    private fun callSmsPhone(parsedResult: SMSParsedResult): () -> Unit = {
        val intent = createCallSmsNumberIntent(parsedResult)
        mStartActivity(intent)
    }

    private fun sendSms(parsedResult: SMSParsedResult): () -> Unit = {
        val intent = createSendSmsToSmsNumberIntent(parsedResult)
        mStartActivity(intent)
    }

    private fun addSmsPhoneNumberToContact(parsedResult: SMSParsedResult): () -> Unit = {
