...
class EmailActionsFragment: AbstractActionsFragment() {
    override fun configureActions(barcode: Barcode, parsedResult: ParsedResult): Array<ActionItem> {
        return when(parsedResult){
            is EmailAddressParsedResult -> configureEmailActions(barcode.contents, parsedResult)
            else -> configureDefaultActions(barcode.contents)
        }
    }

    private fun configureEmailActions(contents: String, parsedResult: EmailAddressParsedResult) = arrayOf(
    ...
    private fun addEmailAddressToContact(parsedResult: EmailAddressParsedResult): () -> Unit = {
        val intent = createAddEmailIntent(parsedResult)
        mStartActivity(intent)
    }

    private fun sendEmail(parsedResult: EmailAddressParsedResult): () -> Unit = {
