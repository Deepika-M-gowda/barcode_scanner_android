...
class BarcodeFormCreatorCode39Fragment: AbstractBarcodeFormCreatorBasicFragment() {
    ...
    override fun generateBarcode() {
        ...
        if(!barcodeFormatChecker.checkCode39Regex(barcodeContents)) {
            configureErrorMessage(getString(R.string.error_barcode_39_regex_error_message))
            return
        }

        hideErrorMessage()
        startBarcodeDetailsActivity(barcodeContents, BarcodeFormat.CODE_39)
    }
