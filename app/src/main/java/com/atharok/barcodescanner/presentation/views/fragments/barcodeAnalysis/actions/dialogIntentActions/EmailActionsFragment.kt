...
class EmailActionsFragment: AbstractDialogIntentActionsFragment() {

    override fun start(barcode: Barcode, parsedResult: ParsedResult) {

        addSearchWithEngineActionFAB(barcode.contents)

        if(parsedResult.type == ParsedResultType.EMAIL_ADDRESS) {
            val actionsArray = arrayOf(ActionEnum.ADD_MAIL, ActionEnum.SEND_MAIL)
            addDialogActionFAB(ActionEnum.OPEN_MAIL_DIALOG, actionsArray, parsedResult)
        }
    }
}